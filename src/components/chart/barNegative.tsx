...
export default function App() {
    ...
    return (
        <Chart theme={chartTheme}>
            <ChartTitle text="Global Ocean Temperature Anomalies" />
            <ChartPrimaryXAxis valueType={'Double'} minimum={1880} maximum={2022} interval={20}>
                <ChartAxisTitle text="Year" />
                <ChartAxisLabel intersectAction="Trim" format={"####"} />
            </ChartPrimaryXAxis>
            <ChartPrimaryYAxis lineStyle={{ width: 0 }} interval={0.2} minimum={-0.5} maximum={0.8}>
                <ChartAxisTitle text="Temperature Anomaly (°C)" />
                <ChartAxisLabel format="{value}°C" rotationAngle={Browser.isDevice ? -45 : 0} />
            </ChartPrimaryYAxis>
            <ChartSeriesCollection>
                <ChartSeries colorField="color" dataSource={rectSeriesNegativeData} xField="Year" yField="Anomaly" type='Bar' cornerRadius={{ bottomRight: 0.5, topRight: 0.5 }} />
            </ChartSeriesCollection>
